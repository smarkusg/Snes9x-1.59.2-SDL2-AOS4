...
//markus	
     S9xMapButton(SDLK_q, S9xGetCommandT("ExitEmu"), false);
     S9xMapButton(SDLK_ESCAPE, S9xGetCommandT("ExitEmu"), false);
     S9xMapButton(SDLK_f, S9xGetCommandT("SDLFullscreen"), false);
...
//markus
		...
		if(event.key.keysym.sym == SDL_MOUSEBUTTONDOWN)
                S9xSDL_toggleFullscreen();
...
//markus
		if(event.key.keysym.sym == SDLK_q)
		S9xReportButton(SDLK_q, false );
//markus
		if(event.key.keysym.sym == SDLK_ESCAPE)
		S9xReportButton(SDLK_ESCAPE, false );
